        ...
        try {
            ...
            for(int i : templates.keySet()){
                ...
                if(!temp.getRequirement().equals("") && !temp.isForCooking() && !blacklist.contains(i)){
                    ...
                    Initiator.debug(temp.getNumber()+": "+temp.getName()+" - "+temp.getDescription()+" ("+temp.getRequirement()+")");
                }
            }
            Initiator.debug("Total achievements loaded into system: "+goodAchievements.size());
        } catch (IllegalAccessException | NoSuchFieldException e) {
            e.printStackTrace();
        }
    }
